...
function buscarProdutos($pdo, $limite = 10) {
    try {
        $sql = "
            SELECT 
                p.id_produto, 
                ...
                p.preco, 
                p.descricao,
                MIN(ip.url_imagem) as url_imagem
            FROM produtos p
...
$produtos_ofertas = buscarProdutos($pdo, 10);
$produtos_destaques = buscarProdutos($pdo, 8);
?>
    <section class="product-showcase">
            ...
            <div class="products-carousel">
                ...
                <?php else: ?>
                    <?php foreach ($produtos_ofertas as $produto): ?>
                        <a href="/livre_mercado/view/Painel_perfil/Painel_vendedor/painel_anúncios/comprar_anuncio.php?id=<?= $produto['id_produto'] ?>" class="product-card">
                            <img src="<?= htmlspecialchars($produto['url_imagem'] ?? '/livre_mercado/imagens/placeholder.png') ?>" alt="<?= htmlspecialchars($produto['nome']) ?>" class="product-image">
                            <div class="product-info">
                                <h3 class="product-name"><?= htmlspecialchars($produto['nome']) ?></h3>
                                <p class="product-price">R$ <?= number_format($produto['preco'], 2, ',', '.') ?></p>
                                <p class="product-installments">em 10x R$ <?= number_format($produto['preco'] / 10, 2, ',', '.') ?> sem juros</p>
                            </div>
                        </a>
                    <?php endforeach; ?>
                <?php endif; ?>
        ...
        <section class="product-showcase">
            <h2>Destaques para você</h2>
             <div class="products-carousel">
                <?php if (empty($produtos_destaques)): ?>
                    <p>Nenhum produto encontrado.</p>
                <?php else: ?>
